...
// ZMĚNA: Importujeme naše nové API a typy
import { findConnection } from '@/api/transportation';
import type { TransportConnection } from '@/api/transportation';
import './BusSchedule.css';

// ZMĚNA: Definujeme si trasy pro jednotlivé děti.
// Toto je konfigurace, kterou můžeme snadno měnit.
const KID_ROUTES = {
  johanka: {
    from: 'Brantice',
    to: 'Zátor, škola',
...
const BusScheduleWidget: React.FC = () => {
  ...
  // ZMĚNA: Nové stavy pro ukládání dat z API, načítání a chyby
  ...
  // ZMĚNA: useEffect pro načítání dat z API při změně dítěte
  useEffect(() => {
    const loadConnection = async () => {
      ...
      try {
        ...
        // API mohlo vrátit 404 (spoj nenalezen) nebo jinou chybu
        console.error(err);
    ...
    loadConnection();
  }, [selectedKid]); // Spustí se znovu při každé změně selectedKid
