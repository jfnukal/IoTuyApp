...
/* ══════════════════════════════════════════════════════════════════════════
   DOČASNÝ ÚKLID — SMAZAT PO POUŽITÍ
   ══════════════════════════════════════════════════════════════════════════
   Filtr nepotravin se opravil 6. 9. 2026, ale působí jen na NOVĚ přijímaná
   data. V `priceDeals` proto pořád leží věci, které se uložily dřív: prací
   prostředky, „Čistič bílý ocet", „Sůl do myčky" a „Mléko pleťové čisticí
   Cien". Samy zmizí, až letákům vyprší `expiresAt` (Firestore TTL) — tohle
   to jen urychlí.

   Proč Cloud Function a ne skript na počítači: generální admin klíč
   k projektu byl 8/2026 zrušen, takže lokální firebase-admin se do databáze
   nedostane. Uvnitř funkcí admin SDK běží sám.

   VÝCHOZÍ REŽIM JE NASUCHO — jen spočítá a vypíše, co by smazal. Doopravdy
   maže až s parametrem `?opravdu=1`.

   Až doběhne: celý tenhle blok odstranit, odebrat export z index.ts, funkci
   smazat z projektu. Mazací endpoint nemá v produkci trvale viset. */
export const uklidNepotraviny = onRequest(
  {
    region: 'europe-west1',
    timeoutSeconds: 540,
    memory: '512MiB',
  ...
  async (req, res) => {
    const ocekavane = process.env.SCRAPER_SECRET;
    if (!ocekavane) {
      res.status(503).json({ chyba: 'Tajemství není nastavené' });
      return;
    }
    if (!sedi(req.get('x-scraper-secret') || '', ocekavane)) {
      console.warn('⚠️ Odmítnuto volání uklidNepotraviny bez platného tajemství.');
      res.status(401).json({ chyba: 'Neplatné tajemství' });
      return;
    }

    const opravdu = req.query.opravdu === '1';
    const db = admin.firestore();

    /* Kolekce se projde po stránkách (`startAfter` na posledním dokumentu).
       Načíst dva tisíce dokumentů najednou by se nemuselo vejít do paměti. */
    const kSmazani: { id: string; nazev: string; duvod: string }[] = [];
    let posledni: FirebaseFirestore.QueryDocumentSnapshot | null = null;
    let prohlednuto = 0;

    for (;;) {
      let dotaz = db.collection(KOLEKCE).orderBy('__name__').limit(500);
      if (posledni) dotaz = dotaz.startAfter(posledni);
      const stranka = await dotaz.get();
      if (stranka.empty) break;

      for (const doc of stranka.docs) {
        prohlednuto++;
        const nazev = String(doc.get('productName') || '');
        if (!nazev) continue;
        /* Přesně týž test jako při příjmu — ať se maže jen to, co by dnes
           stejně dovnitř neprošlo. */
        if (isNonFood(nazev)) {
          kSmazani.push({ id: doc.id, nazev, duvod: 'nepotravina' });
        } else if (!patriNaSeznam(nazev, detectCategory(nazev))) {
          kSmazani.push({ id: doc.id, nazev, duvod: 'nepatří na seznam' });
        }
      }

      posledni = stranka.docs[stranka.docs.length - 1];
      if (stranka.size < 500) break;
    }

    let smazano = 0;
    if (opravdu) {
      for (let i = 0; i < kSmazani.length; i += 400) {
        const cast = kSmazani.slice(i, i + 400);
        const davka = db.batch();
        for (const p of cast) davka.delete(db.collection(KOLEKCE).doc(p.id));
        await davka.commit();
        smazano += cast.length;
      }
    }
    ...
    const souhrn = {
      rezim: opravdu ? 'OSTRY' : 'nasucho',
      prohlednuto,
      nalezeno: kSmazani.length,
      smazano,
      nazvy: kSmazani.map((p) => `${p.nazev} (${p.duvod})`),
    };
    console.log(
      '🧹 uklidNepotraviny:',
      JSON.stringify({ ...souhrn, nazvy: souhrn.nazvy.length })
    );
    res.status(200).json({ ok: true, ...souhrn });
  }
);
